...
// Use gemini-3-pro-preview for complex math tasks
...
export const gradeStudentWork = async (
  ...
  try {
    ...
    // Helper to get correct mime type even if File object is missing/corrupted after reload
    const getMimeType = (f: UploadedFile) => f.mimeType || f.file.type || "application/pdf";

    if (generalAnswerKey) {
        parts.push({ text: `\n--- ĐÁP ÁN CHUNG ---` });
        parts.push({ inlineData: { mimeType: getMimeType(generalAnswerKey), data: generalAnswerKey.base64 } });
    }

    questions.forEach((q) => {
      if (q.file) {
        parts.push({ text: `\n--- ĐÁP ÁN CÂU ${q.label} ---` });
        parts.push({ inlineData: { mimeType: getMimeType(q.file), data: q.file.base64 } });
      }
    });

    if (referenceFile) {
        parts.push({ text: `\n--- TÀI LIỆU THAM KHẢO ---` });
        parts.push({ inlineData: { mimeType: getMimeType(referenceFile), data: referenceFile.base64 } });
    }

    parts.push({ text: `\n--- BÀI LÀM HỌC SINH ---` });
    studentWorkFiles.forEach((file) => {
      parts.push({ inlineData: { mimeType: getMimeType(file), data: file.base64 } });
    });
...
export const gradePracticeWork = async (
    ...
    try {
        ...
        const getMimeType = (f: UploadedFile) => f.mimeType || f.file.type || "image/jpeg";
